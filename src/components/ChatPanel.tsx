...
export function ChatPanel({ phase, run, context, history, onMessage, fixedNpc, label = "自由对话", onAttachment }: { phase: number; run: number; context: string[]; history: ChatMessage[]; onMessage: (npc: NpcId, text: string, reply: string, intent?: IntentId, event?: WorldEvent) => void; fixedNpc?: NpcId; label?: string; onAttachment?: (id: string) => void }) {
  ...
  return <section className={`chat-panel ${fixedNpc ? "inline-chat" : "card"}`}><h3>{label}</h3>{!fixedNpc && <div className="actions">{npcs.map(([id, npcLabel]) => <button key={id} className={npc === id ? "" : "secondary"} onClick={() => setNpc(id)}>{npcLabel}</button>)}</div>}<div className="chat-history">{thread.map((item, index) => <div className={`chat-message ${item.role}`} key={`${item.role}-${index}`}><span>{item.role === "user" ? "你：" : `${npc}：`}{item.text}<small>刚刚</small></span>{item.attachmentId && <button className="attachment-card" onClick={() => onAttachment?.(item.attachmentId!)}>📎 {item.attachmentId}</button>}</div>)}{reply && !thread.some((item) => item.role === "assistant" && item.text === reply) && <p className="chat-message assistant">{npc}：{reply}<small>刚刚</small></p>}</div><textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="输入一句话……" maxLength={300} /><div className="actions"><button onClick={send} disabled={busy || !message.trim()}>{busy ? "对方正在输入……" : "发送"}</button></div></section>;
}

  ...
  return <>
    ...
    <main className="layout">
      <div>
        ...
        {state.phase >= 2 && <CommentSection state={state} open={() => action("OPEN_FOLDED_COMMENTS")} reply={() => action("REPLY_USER_404")} />}
        ...
        {state.phase >= 4 && <EvidencePanel state={state} view={(clueId) => action("VIEW_CLUE", { clueId })} />}
        {state.currentRun.draftAvailable && <DraftPanel state={state} open={() => action("OPEN_DRAFT")} publish={() => action("PUBLISH_ANSWER")} />}
        <RiskChoicePanel state={state} choose={(node, choice) => action("CHOOSE_RISK", { node, choice })} />
        <OwnAnswerCard state={state} />
        <EndingPanel state={state} choose={(endingId) => action("CHOOSE_ENDING", { endingId })} triggerExit={(endingId) => action("TRIGGER_EXIT", { endingId })} confirm={() => action("CONFIRM_ENDING")} cancel={() => action("CANCEL_ACTION")} advance={() => action("ADVANCE_ENDING_SCREEN")} reenter={() => action("REENTER_NEXT_RUN")} retry={() => action("RETRY_AFTER_MELTDOWN")} />
      </div>
      <aside className="side">
        <GlitchLayer pollution={state.pollution} stopped={state.timeStopped} />
        {state.phase >= 3 && <RulePanel state={state} view={(ruleId) => action("VIEW_RULE", { ruleId })} />}
        <section className="card"><h3>当前周目</h3><div className="meta">第 {state.run} 周目 · {pollutionBand(state.pollution) === "stable" ? "页面稳定" : pollutionBand(state.pollution) === "unstable" ? "页面不稳定" : "页面失控"}</div><div className="actions"><button className="ghost" onClick={state.reset}>重置存档</button>{state.previousRun && <button className="ghost" onClick={() => action("CLEAR_PREVIOUS_RUN")}>清除上一轮记录</button>}</div></section>
